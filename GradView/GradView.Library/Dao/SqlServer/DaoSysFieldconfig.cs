...
namespace GradView.Library.Data.SqlServer
{
	...
	partial class DaoSysFieldconfig:IDataBaseOperate<SysFieldconfig>
	{
		...
		private SqlParameter[] GetParms(SysFieldconfig model)
		{
			...
			parms[14].Value = model.Ipxh;	
			parms[15].Value = model.Regextypeid;	
			if(model.Edittypeid == null)
				parms[16].Value =  DBNull.Value;
			else
				parms[16].Value = model.Edittypeid;
			if(model.Regrxtext == null)
				parms[17].Value =  DBNull.Value;
			else
				parms[17].Value = model.Regrxtext;
			parms[18].Value = model.Keytableid;	
			if(model.Isedit == null)
				parms[19].Value =  DBNull.Value;
			else
				parms[19].Value = model.Isedit;
			parms[20].Value = model.Editsort;	
			parms[21].Value = model.Editminlength;	
			parms[22].Value = model.Editmaxlength;	
