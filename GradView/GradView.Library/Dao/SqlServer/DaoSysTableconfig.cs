...
namespace GradView.Library.Data.SqlServer
{
	...
	partial class DaoSysTableconfig:IDataBaseOperate<SysTableconfig>
	{
		...
		{
			string sql = "INSERT INTO sys_TableConfig (tableid,tableName,tableNameCh,ShowStyle,EditStyle,isDel,isEdit,isPage,isAllSelect,isSort,isCustomColumn,CustomColumnName) VALUES (@tableid,@tableName,@tableNameCh,@ShowStyle,@EditStyle,@isDel,@isEdit,@isPage,@isAllSelect,@isSort,@isCustomColumn,@CustomColumnName)";
		...
		private SqlParameter[] GetParms(SysTableconfig model)
		{
			SqlParameter[] parms = {
				...
				new SqlParameter("@ShowStyle", SqlDbType.VarChar, 100),			
				new SqlParameter("@EditStyle", SqlDbType.VarChar, 100),			
				...
				new SqlParameter("@isSort", SqlDbType.VarChar, 50),			
			...
			if(model.Showstyle == null)
				parms[3].Value =  DBNull.Value;
			else
				parms[3].Value = model.Showstyle;
			if(model.Editstyle == null)
				parms[4].Value =  DBNull.Value;
			else
				parms[4].Value = model.Editstyle;
			if(model.Isdel == null)
				parms[5].Value =  DBNull.Value;
			else
				parms[5].Value = model.Isdel;
			if(model.Isedit == null)
				parms[6].Value =  DBNull.Value;
			else
				parms[6].Value = model.Isedit;
			if(model.Ispage == null)
				parms[7].Value =  DBNull.Value;
			else
				parms[7].Value = model.Ispage;
			if(model.Isallselect == null)
				parms[8].Value =  DBNull.Value;
			else
				parms[8].Value = model.Isallselect;
			if(model.Issort == null)
				parms[9].Value =  DBNull.Value;
			else
				parms[9].Value = model.Issort;
			if(model.Iscustomcolumn == null)
				parms[10].Value =  DBNull.Value;
			else
				parms[10].Value = model.Iscustomcolumn;
			if(model.Customcolumnname == null)
				parms[11].Value =  DBNull.Value;
			else
				parms[11].Value = model.Customcolumnname;
			parms[12].Value = model.Ipxh;	
		...
		{
		    string sql= "UPDATE sys_TableConfig SET tableName=@tableName, tableNameCh=@tableNameCh, ShowStyle=@ShowStyle, EditStyle=@EditStyle, isDel=@isDel, isEdit=@isEdit, isPage=@isPage, isAllSelect=@isAllSelect, isSort=@isSort, isCustomColumn=@isCustomColumn, CustomColumnName=@CustomColumnName WHERE 1=1  AND tableid=@tableid";
		...
		private SysTableconfig LoadModel(IDataReader dr)
		{
			SysTableconfig model = new SysTableconfig();
				...
				if (dr["ShowStyle"] != DBNull.Value)
					model.Showstyle	 = dr["ShowStyle"].ToString();	
				if (dr["EditStyle"] != DBNull.Value)
					model.Editstyle	 = dr["EditStyle"].ToString();	
				...
				if (dr["isSort"] != DBNull.Value)
					model.Issort	 = dr["isSort"].ToString();	
