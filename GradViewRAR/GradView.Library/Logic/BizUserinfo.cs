...
{	
	...
	{
		private static IDataBaseOperate< Userinfo> baseOperate =(IDataBaseOperate< Userinfo>) DaoFactory.CreateInstance("DaoUserinfo");
		
		#region BaseOperation
		
		/// <summary>
		/// 在dbo.UserInfo中增加一条记录
		/// </summary>
		...
		/// <returns>影响行数</returns>
		[DataObjectMethod(DataObjectMethodType.Insert)]
		public static int Insert(Userinfo model,object trans)
		{
			if(model != null && trans !=null)
				return baseOperate.Insert(model,trans);
			else
			 	throw new ApplicationException("在dbo.UserInfo中增加一条记录时,传递的参数model,trans有错误！");;
		}
		...
		public static int  Update(Userinfo model)
		{
			if(model != null)
				return baseOperate.Update(model);
				 ...
				 throw new ApplicationException("在dbo.UserInfo中更新一条记录时,传递的参数model为Null！");
		}
		
		/// <summary>
		/// 在dbo.UserInfo中完整更新一条记录,支持数据库事务
		/// </summary>
		/// <param name="model">包含被更新数据的实体对象</param>
		/// <param name="trans">事务参数</param>
		/// <returns>影响行数</returns>
		[DataObjectMethod(DataObjectMethodType.Update)]
		public static int Update(Userinfo model,object trans)
		{
			if(model != null && trans !=null)
				return baseOperate.Update(model,trans);
			else
			   throw new ApplicationException("在dbo.UserInfo中更新一条记录时,传递的参数model,trans有错误！");
		}
      	...
      	public static int Update(string sql, object[] parms)
		{
			if(sql != null && parms !=null)
			   ...
			   throw new ApplicationException("根据指定的Sql在dbo.UserInfo中更新一条记录时,传递的参数sql,parms有错误！");
		}
      	...
      	public static int Update(string sql, object[] parms, object trans)
		{
			if(sql != null && parms !=null && trans !=null)
			   ...
			   throw new ApplicationException("根据指定的Sql在dbo.UserInfo中更新一条记录时,传递的参数sql,parms,trans有错误！");
		}
		...
		[DataObjectMethod(DataObjectMethodType.Select)]
      	public static Userinfo  GetModel(string sql,object[] parms)
		{
			if(!string.IsNullOrEmpty(sql))
				return baseOperate.GetModel(sql,parms);	
			else
				throw new ApplicationException("根据指定的SQL获取dbo.UserInfo中的一条记录时,传递的参数sql为Null！");
		}
		...
		/// <returns>记录集</returns>
		[DataObjectMethod(DataObjectMethodType.Select)]
		public static IList<Userinfo> GetList(string sql,object[] parms)
		{
			if(!string.IsNullOrEmpty(sql))
				return baseOperate.GetList(sql,parms);	
			else
				throw new ApplicationException("根据指定的SQL在dbo.UserInfo中获取所有记录时,传递的参数sql为Null！");
		}	
